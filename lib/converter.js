...
function sortModulesByDep(modules, main) {
    ...
    if (nodeList.length) {
        var circles = nodeList.map(function (item) {
            return item.id;
        });
        console.warn('modules exist circle dependencies: ' + circles.join('; '));
 ...
 * @param {string=} options.exports 把所有模块对象导出到全局的变量名，可选，默认不导出
 * @return {string}
 */
function converter(options) {
    ...
    var combineIFFECode = generator.getCombineIFFECodes(modules, options.exports);
    ...
    if (options.output) {
        ...
        fs.writeFileSync(outputPath, combineIFFECode, 'utf8');
    }
