 ...
 * @return {Object}
 */
function createDepGraph(modules) {
    var nodeList = [];
    var nodeMap = {};
...
function sortModulesByDep(modules, main) {
    var data = createDepGraph(modules);
    var nodeList = data.nodeList;
    var found;
    var sortedModules = [];
    var preferNode = data.nodeMap[main];
    ...
    if (nodeList.length) {
        throw new Error('Unexpected circle deps cannot resolve');
    }
...
function converter(options) {
    ...
    sortModulesByDep(modules, options.main);
